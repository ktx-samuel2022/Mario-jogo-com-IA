...
function startGame() {
  GameStatus = "start";
  document.getElementById("status").innerHTML = "Jogo está carregando...";
}
...
function changeGameStatud(character) {
  if (NarizX != 0 && gameConfig.status === "start" && GameStatus == "start") {
    document.getElementById("status").innerHTML = "O jogo foi carregado";
    somInicio.setVolume(0.5)
    somInicio.play()
...
function jumping(character) {
  if (
    (NarizY < 200 && character.live) ||
    (touchIsDown && character.live)
  ) {
    character.velocity.y += gameConfig.jump;
    somPular.setVolume(0.5)
    somPular.play()
  }
}
